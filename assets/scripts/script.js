...
const dataPaths = {
  ...
  teachingRecognition: "data/teaching/recognition/recognition.json",
  taughtCourses: "data/teaching/courses/courses.json",
  probabilityStochasticProcessesLectures:
    "data/teaching/courses/probability-stochastic-processes/lectures.json",
  probabilityStochasticProcessesPracticeSets:
    "data/teaching/courses/probability-stochastic-processes/practice-sets.json",
};

const isNestedCoursePage = /\/pages\/courses\/[^/]+\.html?$/.test(window.location.pathname);
const isPagesPath = /\/pages\/[^/]+\.html?$/.test(window.location.pathname);
const basePrefix = isNestedCoursePage ? "../../" : isPagesPath ? "../" : "";
...
function renderPracticeSetsTable(sets) {
  ...
  let html = "<thead><tr><th>P. Set No.</th><th>Problem Set Contents</th><th>Course Material</th></tr></thead><tbody>";
  sets.forEach((item) => {
    const materialHtml = item.pdfHref
      ? "<div class=\"award-entry course-material-entry\">" + actionButtonsHtml({ pdf: item.pdfHref, web: "" }) + "</div>"
      : "<div class=\"award-entry course-material-entry\">PDF pending</div>";
    ...
    html +=
      "<tr><td data-label=\"Problem Set No.\" class=\"lecture-no-cell\"><div class=\"award-entry lecture-no-entry\">" +
      escapeHtml(item.number) +
      "</div></td><td data-label=\"Problem Set Content\" class=\"lecture-title-cell\"><div class=\"award-entry lecture-title-entry\">" +
  ...
  try {
    ...
    const normalizedTeaching = teachingRows.map((item) => ({
      ...
      pdfHref: item["PDF Link"]
        ? resolveFromRoot(
          `data/teaching/recognition/files/${String(item["PDF Link"]).trim()}`
        )
        : "",
    }));
...
async function loadPracticeSetsTable() {
  try {
    ...
    const normalized = rows.map((item, index) => ({
      number: String(item["Problem Set No."] ?? item["Practice Set No."] ?? index + 1).trim(),
